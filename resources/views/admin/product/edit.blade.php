...
<form action="{{ route('admin.product.update', $ed->id) }}" method="POST" role="form" enctype='multipart/form-data' >
    
    @csrf
    <div class="form-group">
        <label for="">Tên sản phẩm</label>
        <input type="text" name="name" class="form-control" value="{{$ed->name}}" id="name" placeholder="Input field">   
    </div>
    @if($errors->has('name'))
    ...
    <div class="form-group">
        ...
        <input type="text" name="slug" class="form-control" value="{{$ed->slug}}" id="slug" placeholder="Input field">  
    </div>
    @if($errors->has('slug'))
	...
	<div class="form-group">
        <label for="">Giá</label>
        <input type="text" name="price" class="form-control" value="{{$ed->price}}" id="" placeholder="Input field">  
	</div>
    @if($errors->has('price'))
	...
	<div class="form-group">
        ...
        <input type="text" name="sale_price" class="form-control" value="{{$ed->sale_price}}" id="" placeholder="Input field">  
    </div>
    ...
    <div class="form-group">
            ...
            <br>
            <textarea rows = "3" cols = "60" id="content" name = "shortdes">{{$ed->shortdes}}</textarea>
            </br>  
    </div>
	...
	<div class="form-group">
		...
		<br>
        <textarea rows = "5" cols = "60" id="content" name = "description">{{$ed->description}}</textarea>
		</br>  
    </div>
